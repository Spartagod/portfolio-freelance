...
export default function ExperienceSection() {
  return (
    <section
      id="experience"
      className="relative min-h-screen bg-gradient-to-b from-black to-purple-950 flex flex-col items-center justify-start px-4 sm:px-10 xl:px-24 overflow-hidden"
    >
      <div className="flex flex-col items-center justify-start w-full z-10" style={{ paddingTop: '15vh' }}>
        <h2 className="text-white text-center text-4xl md:text-5xl font-semibold mb-12">
          Experience
        </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 w-full">
          {experiences.map((exp, index) => (
            ...
            >
              <h3 className="text-purple-400 text-xl md:text-lg font-semibold mb-2">{exp.title}</h3>
              <p className="text-gray-300 text-sm mb-2">{exp.company}</p>
              <p className="text-gray-400 text-xs mb-4">{exp.period}</p>
              <p className="text-gray-200 text-sm md:text-base">{exp.description}</p>
            </motion.div>
          ))}
        </div>
      </div>

      <svg viewBox="0 0 1440 200" preserveAspectRatio="none" className="absolute bottom-0 w-full h-48 opacity-20 z-0">
